...
traveller <- trajectory("journey") %>%
  ...
  trajectory("City1")%>%
    ...
    set_attribute("Current", function() {sample(c(1,2,3), 1, prob = c(0,0.6,0.4))})  %>% #INITIALIZE with Next step
    log_("Leaving City1")%>%
    timeout(1) %>%
    release("City1") ,
  ...
  trajectory("City2")%>%
    ...
    set_attribute("Steps",1,mod="+")    %>%                                        #Increment Steps
    set_attribute("Current", function() {sample(c(1,2,3), 1, prob = c(0.7,0,0.3))})  %>% #INITIALIZE with Next step
    log_("Leaving City2")%>%
    timeout(1) %>%
    release("City2"),
  ...
  trajectory("City3")%>%
    ...
    set_attribute("Steps",1,mod="+")    %>%                                        #Increment Steps
    set_attribute("Current", function() {sample(c(1,2,3), 1, prob = c(0.5,0.5,0))})  %>% #INITIALIZE with Next step
    log_("Leaving City3")%>%
    timeout(1) %>%
    release("City3"),
...
plot(get_mon_resources(paths),metric="usage")
