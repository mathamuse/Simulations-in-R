...
envs <- lapply(1:no_replications,function(i){
  ...
  # Print the M2_attempt distribution
...
print(p)
q<- plot(arr_data,metric="flow_time")
print(q)
t<-plot(arr_data,metric="waiting_time")
print(t)

overall_data <- do.call(rbind,envs)
overall_data
overall_average_pass_rate <-sum(unlist(overall_data[,4]))/no_replications
overall_average_pass_rate
overall_average_fail_rate <-sum(unlist(overall_data[,5]))/no_replications
overall_average_fail_rate
overall_average_scrap_rate <-sum(unlist(overall_data[,6]))/no_replications
overall_average_scrap_rate
